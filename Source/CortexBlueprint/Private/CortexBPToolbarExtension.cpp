...
FCortexConversionPayload FCortexBPToolbarExtension::CapturePayload(TSharedPtr<FBlueprintEditor> Editor)
{
	...
	for (const FBPInterfaceDescription& IfaceDesc : Blueprint->ImplementedInterfaces)
	{
		...
		// A Blueprint interface is one generated by a UBlueprint; a native UInterface has no
		// generator. Same test as bParentIsBlueprint above. The previous "/Game/" prefix check
		// misclassified every Blueprint interface living in a plugin as native.
		Info.bIsBlueprint = (IfaceDesc.Interface->ClassGeneratedBy != nullptr);
